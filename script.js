...
botonInicio.addEventListener('click', () => {
  ...
  reproducirMusica();
  setTimeout(() => document.querySelectorAll('.ramo').forEach(r => r.classList.add('ramo--visible')), TIEMPO_APARICION_RAMO);
}, { once: true });

// Pausa la música al salir de la pestaña o bloquear el teléfono, y la retoma al volver.
let musicaSonabaAntes = false;
document.addEventListener('visibilitychange', () => {
  if (document.hidden) { musicaSonabaAntes = !audio.paused; audio.pause(); }
  else if (musicaSonabaAntes) reproducirMusica();
});
